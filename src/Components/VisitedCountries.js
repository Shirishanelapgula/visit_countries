...
class VisitedCountries extends Component {
  ...
  addCountryToList = id => {
    ...
    newCountriesList[index] = item

    this.setState({countriesList: newCountriesList})
  }

  removeCountryToList = id => {
    const {countriesList} = this.state
        ...
        <div>
          ...
          {visitedCountries.length > 0 ? (
            <ul className="bottom-container">
              {visitedCountries.map(every => (
                <Countries
                  key={every.id}
                  data={every}
                  removeCountryToList={this.removeCountryToList}
                />
              ))}
            </ul>
          ) : (
            visitedCountries.length === 0 && (
              <p className="head">No Countries Visited Yet</p>
            )
          )}
        </div>
      </div>
